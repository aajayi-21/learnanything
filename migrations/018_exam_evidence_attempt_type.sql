-- Allow the 'exam_evidence' attempt type (exam seeding: backdated, discounted
-- attempts imported from a past practice exam). SQLite cannot alter a CHECK
-- constraint in place, so rebuild practice_attempts following the 004 pattern,
-- preserving the current table shape (004 columns plus the session/scheduler
-- columns added by 010).
